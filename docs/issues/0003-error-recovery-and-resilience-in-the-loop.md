...
- [ ] When a research subagent times out (via `spawnSubagent` timeout or `ctx.signal` abort), the iteration loop catches the error, logs it to `state.md` under a "## Errors" section (agent type, agent ID, error message, timestamp), and continues to the next iteration
- [ ] When a subagent returns an empty or malformed output, the loop detects it, logs a warning to `state.md`, and does not crash the research session
- [ ] On subagent failure, the loop retries once with the same agent type before giving up and moving on; the retry is recorded in state
- [ ] `wasCompleteToolCalled()` logs errors instead of silently swallowing them (at minimum a `console.warn`)
- [ ] `archiveLatestSubagentOutput()` logs errors instead of silently swallowing them and returns a structured result indicating success/failure
- [ ] After max iterations with persistent failures, state.md is marked with a "## Status\npartial" status and the user is notified via `ctx.ui.notify()` with a clear message about what failed
